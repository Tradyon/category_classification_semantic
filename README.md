...
## Extract HS codes
- Gemini direct:  
  `.venv/bin/python extract_hs_codes_gemini.py --input "HS Code" --pages-jsonl outputs/hs_pages_gemini.jsonl --rows-csv outputs/hs_rows_gemini.csv`  
...
2) Index HS + attributes:  
   `.venv/bin/python weaviate_index.py --hs-csv outputs/hs_rows_gemini.csv --attrs-json attrs.json --recreate`
...
`.venv/bin/python weaviate_index.py --recreate --llm-augment --llm-mode keywords --llm-threshold 80`  
...
`.venv/bin/python weaviate_index.py --recreate --llm-augment-attrs`  
...
`.venv/bin/python weaviate_search.py "organic green coffee"`  
...
## Frontend (port 8000)
Start a minimal UI:  
`.venv/bin/python web_frontend.py --host 127.0.0.1 --port 8000`  
